...
import { holdings as seedHoldings, portfolios } from "@/lib/mock-data";
...
function HoldingsPage() {
  ...
  const holdingsWithPortfolio = useMemo(
    () =>
      holdings.map((h, i) => ({
        ...h,
      ...
      {isLoading ? (
        ...
        <Card className="p-6 text-center">
          <div className="text-sm text-destructive mb-3">
            Could not load holdings from the API.
          </div>
        ...
        <Card className="card-elevated overflow-hidden">
          <div className="flex flex-wrap items-center gap-2 p-4 border-b">
            ...
            <div className="ml-auto text-xs text-muted-foreground">Showing {filteredHoldings.length} of {holdings.length}</div>
          </div>
                ...
                {filteredHoldings.length === 0 ? (
                  ...
                  return (
                    <TableRow key={h.stocks.symbol} className="cursor-pointer group">
                      <TableCell>
                        <Link to="/stock/$symbol" params={{ symbol: h.stocks.symbol }} className="flex items-center gap-3 group-hover:text-primary">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-muted-foreground truncate">{h.stocks.fullName}</div>
                          </div>
                        </Link>
                      </TableCell>
                      <TableCell><Badge variant="secondary" className="font-normal">{h.stocks.sector.name}</Badge></TableCell>
                      ...
                      <TableCell className="text-right tabular-nums font-medium">{Number(h.stocks.currentPrice).toFixed(2)}</TableCell>
                      <TableCell className="text-right tabular-nums font-semibold">{mv.toLocaleString(undefined, { maximumFractionDigits: 0 })}</TableCell>
                      <TableCell className="text-right tabular-nums">{h.stocks.dividendYield}%</TableCell>
                      <TableCell className="text-right tabular-nums text-primary">{yieldOnCost}%</TableCell>
                      <TableCell className="text-right tabular-nums">{h.stocks.annualDividend}</TableCell>
